...
abstract class SimpleFragment : Fragment() {
    ...
    protected// load DataSets from files in assets folder
    val complexity: LineData
        get() {

            val sets = ArrayList<ILineDataSet>()

            val ds1 = LineDataSet(FileUtils.loadEntriesFromAssets(context!!.assets, "n.txt"), "O(n)")
            val ds2 = LineDataSet(FileUtils.loadEntriesFromAssets(context!!.assets, "nlogn.txt"), "O(nlogn)")
            val ds3 = LineDataSet(FileUtils.loadEntriesFromAssets(context!!.assets, "square.txt"), "O(n\u00B2)")
            val ds4 = LineDataSet(FileUtils.loadEntriesFromAssets(context!!.assets, "three.txt"), "O(n\u00B3)")
    ...
    protected fun generateScatterData(dataSets: Int, range: Float, count: Int): ScatterData {
        ...
        val shapes = ScatterChart.ScatterShape.getAllDefaultShapes()
    ...
    protected fun generateLineData(): LineData {
        ...
        val sets = ArrayList<ILineDataSet>()
        val ds1 = LineDataSet(FileUtils.loadEntriesFromAssets(context!!.assets, "sine.txt"), "Sine function")
        val ds2 = LineDataSet(FileUtils.loadEntriesFromAssets(context!!.assets, "cosine.txt"), "Cosine function")
